...
setwd("~/Dropbox (Cambridge University)/Bioinformatics_core/Projects/Samuel_Furse_Project/Further_analyses_with_V2.0/PWD_project/Results-minus_ve/")# <--- insert here the path to the working (output) directory

new_wd <- getwd()

#Sets the input directory
inputdir <-"~/Dropbox (Cambridge University)/Bioinformatics_core/Projects/Samuel_Furse_Project/Further_analyses_with_V2.0/PWD_project/PWD dataset_2020-11-04/"# <--- insert here the path to the input directory
...
for(j in 1: length(tissues)){
  ...
  if(length(aa)!=0){
    ...
    cc_1_nozeroes <- cc_1[-which(cc_1_zeroes > ncol(cc_1)*20/100) ,]
    cc_2_nozeroes <- cc_2[-which(cc_2_zeroes > ncol(cc_2)*20/100) ,]
